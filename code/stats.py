...
def analyze_CONTENT(CONTENT):
    result = {}
    result['MENTIONS'] = CONTENT.count('USER_MENTION')
    result['URLS'] = CONTENT.count('URL')
    result['POS_EMOS'] = CONTENT.count('EMO_POS')
    result['NEG_EMOS'] = CONTENT.count('EMO_NEG')
    CONTENT = CONTENT.replace('USER_MENTION', '').replace(
        'URL', '')
    words = CONTENT.split()
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        ...
        exit()
    num_CONTENT, num_pos_CONTENT, num_neg_CONTENT = 0, 0, 0
    ...
    with open(sys.argv[1], 'r') as csv:
        lines = csv.readlines()
        num_CONTENT = len(lines)
        for i, line in enumerate(lines):
            t_id, if_pos, CONTENT = line.strip().split(',')
            if_pos = int(if_pos)
            if if_pos:
                num_pos_CONTENT += 1
            else:
            	num_neg_CONTENT += 1
            result, words, bigrams = analyze_CONTENT(CONTENT)
    ...
    print '\n[Analysis Statistics]'
    print 'Tweets => Total: %d, Positive: %d, Negative: %d' % (num_CONTENT, num_pos_CONTENT, num_neg_CONTENT)
    print 'User Mentions => Total: %d, Avg: %.4f, Max: %d' % (num_mentions, num_mentions / float(num_CONTENT), max_mentions)
    print 'URLs => Total: %d, Avg: %.4f, Max: %d' % (num_urls, num_urls / float(num_CONTENT), max_urls)
    print 'Emojis => Total: %d, Positive: %d, Negative: %d, Avg: %.4f, Max: %d' % (num_emojis, num_pos_emojis, num_neg_emojis, num_emojis / float(num_CONTENT), max_emojis)
    print 'Words => Total: %d, Unique: %d, Avg: %.4f, Max: %d, Min: %d' % (num_words, num_unique_words, num_words / float(num_CONTENT), max_words, min_words)
    print 'Bigrams => Total: %d, Unique: %d, Avg: %.4f' % (num_bigrams, num_unique_bigrams, num_bigrams / float(num_CONTENT))
