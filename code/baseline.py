...
# Classifies a comment based on the number of positive and negative words in it
...
def classify(processed_csv, test_file=True, **params):
    ...
    with open(processed_csv, 'r') as csv:
        for line in csv:
            if test_file:
                CONTENT_id, CONTENT = line.strip().split(',') 
            else:
                CONTENT_id, label, CONTENT = line.strip().split(',')
            ...
            if test_file:
                predictions.append((CONTENT_id, prediction))
            else:
                predictions.append((CONTENT_id, int(label), prediction))
    return predictions
