...
#Glove seeds can be dowloaded via link below
...
def process_CONTENT(csv_file, test_file=False):
    CONTENT = []
    labels = []
...
if __name__ == '__main__':
    ...
    CONTENT,labels = process_CONTENT(TRAIN_PROCESSED_FILE, test_file=True)
    ...
    if train:
        ...
        print model.summary()
        model.fit(CONTENT, labels, batch_size=128, epochs=5, validation_split=0.1, shuffle=True, callbacks=[checkpoint, reduce_lr])
    else:
